...
* [Utils](#utils-api)
    - [isUnderIframe()](#utilsisunderiframe-method)
...
> 
> Btw, please check our [Deal with AdBlock](#deal-with-adblock) section below. 
...
> *Note: `license.XX.js` is file i sent you via email after you registered your domains with us. Don't use license file that you got from updates or .zip file, it's just a demo license and does not work in production environment.*
...
> To change this variable, please add `window.popns = 'YourOwnName'` on top of `script.js` or before loading `script.js`
> For example:
> ```
...
> Recently AdBlock blocked our variable `popns` that we used to change BetterJsPop variable name, it also blocked `BetterJsPop`. So whole script will not able to run because of this. 
> 
> To prevent them easily block all us, we provided new `script.php` file that used to combine both `script.js` and your `license.XX.js` to one file, it also automatic change `.popns` to a random name so AdBlock can't block it. We also recommend to change `BetterJsPop` or `YourOwnName` to another name.
> 
> Then now we can use like this:
> ```
> <script src="http://domain.com/script/script.php"></script>
> <script>YourOwnName.add('http://example.com');</script>
> ```
> 
> If you don't want to use PHP, you can use our [Merger Tool](http://popunderjs.com/merge.php) to do all above things then output to a single .js file for you.
> 
> Or if you want to do it manually, just open `script.js` and your `license.XX.js` then find and replace `.popns` to a random string such as `.aRandomString` (only a-z characters)
...
| [**`tabUnderIgnoreTargetBlank`**](#option-tabunderignoretargetblank) | `boolean`           | `false`                         |
| [**`tricksChrome`**](#option-trickschrome)                           | `boolean`           | `false`                          |
...
##### Option: `tabUnderIgnoreTargetBlank`

`@since 2.7.28`

When performing tab-under, by defaults:

- For a link **with target=_blank**, script will open current url (window.location.href) in new tab, then redirect old tab to pop url. [1]

- For a link **without target=_blank**, script will open link inside `href` attribute in new tab, then redirect old tab to pop url. [2]


But if you set config `tabUnderIgnoreTargetBlank` to true, it will performs both cases same as without target=_blank [2]
...
Try to make a pop-up become pop-under by performs tricking in Chrome browser when real popunder is not available.
...
* **`coookieDomain`** *type `string|null`, default `null`*
    - cookieDomain for this pop, you can use to set cookie for `.yourdomain.com`
 ...
 * @param string|null
 */
BetterJsPop.Cookie.set(name, value, expires, path, domain);
```
...
#### `Utils.isUnderIframe()` method
```
/**
 * Used to determine if pop is placed inside an iframe
 *
 * @return boolean
 */
BetterJsPop.Utils.isUnderIframe();
```
